...
class SDSRequest extends SDSMessage_1.SDSMessage {
    ...
    set oId(oId) {
        ...
        (0, network_1.htonl)(this.buffer, 0, parseInt(splittedId[0], 10));
        (0, network_1.htonl)(this.buffer, 4, parseInt(splittedId[1], 10));
    }
    /**
    ...
    addParameter(parameterName, value) {
        if (parameterName === SDSMessage_1.ParameterNames.OTHER_OBJECT_ID) {
            this.addOIDParameter(parameterName, value);
        }
        else if (Array.isArray(value)) {
            this.addStringList(parameterName, value);
        }
        else if (value === null) {
            this.addNullFlag(parameterName);
        }
        else {
            switch (typeof value) {
                case "number":
                    this.addInt32(parameterName, value);
                    break;
                case "string":
                    this.addString(parameterName, value);
                    break;
                case "boolean":
                    this.addBoolean(parameterName, value);
                    break;
                default:
    ...
    pack() {
        ...
        (0, network_1.htonl)(message, 0, messageSize);
     ...
     * @param value Value of the parameter
     */
    addBoolean(parameterName, value) {
        let type = SDSMessage_1.Types.BOOLEAN;
        // special case: if the boolean value is "false", we have to pass a null flag (bitwise or) o.O
        if (!value) {
            type |= SDSMessage_1.Types.NULL_FLAG;
        }
    ...
    addInt32(parameterName, value) {
        this.add([SDSMessage_1.Types.INT32, parameterName]);
        ...
        (0, network_1.htonl)(bytes, 0, value);
        this.add(bytes);
    }
    addNullFlag(parameterName) {
        this.add([SDSMessage_1.Types.NULL_FLAG, parameterName]);
    }
    /**
     ...
     * @param oIdLow
     */
    addOIDParameter(parameterName, oId) {
        this.add([SDSMessage_1.Types.OID, parameterName]);
        // note: we can't directly pass "this.buffer" to the htonl function because we don't know if
        // the buffer has free space left
        const bytes = Buffer.from([0, 0, 0, 0]);
        const splitted = oId.split(":");
        const oIdHigh = parseInt(splitted[0], 10);
        (0, network_1.htonl)(bytes, 0, oIdHigh);
        this.add(bytes);
        const oIdLow = parseInt(splitted[1], 10);
        (0, network_1.htonl)(bytes, 0, oIdLow);
        this.add(bytes);
    }
    /**
    ...
    addString(parameterName, value) {
        this.add([SDSMessage_1.Types.STRING, parameterName]);
        // [..., string size in network byte order, the string itself]
        const stringSize = Buffer.from([0, 0, 0, 0]);
        (0, network_1.htonl)(stringSize, 0, SDSMessage_1.SDSMessage.length_term_utf8(value));
        this.add(stringSize);
        this.add(SDSMessage_1.SDSMessage.term_utf8(value));
    }
    /**
     ...
     * @param value Value of the parameter
     */
    addStringList(parameterName, value) {
        // head-part of parameter
        this.add([SDSMessage_1.Types.STRING_LIST, parameterName]);
        /*
        ...
        */
        ...
        (0, network_1.htonl)(networkByteOrderBuffer, 0, value.length);
        ...
        for (const element of value) {
            ...
            const encodedElement = SDSMessage_1.SDSMessage.term_utf8(element);
            (0, network_1.htonl)(networkByteOrderBuffer, 0, encodedElement.length);
            this.add(networkByteOrderBuffer);
        ...
        (0, network_1.htonl)(this.buffer, dataSizeIndex, dataPartSize);
    }
}
exports.SDSRequest = SDSRequest;
//# sourceMappingURL=SDSRequest.js.map