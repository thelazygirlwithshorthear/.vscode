...
const extractSingleLineComments = (contents, file, lineCommentRegex) => {
    const comments = [];
    (0, eol_1.split)(contents).forEach((line, index) => {
        let match = lineCommentRegex.exec(line);
        while (match && match.length > 0) {
            const comment = (0, exports.prepareComment)(match, index + 1, file);
            if (!comment) {
                break;
            }
...
const extractSingleLineFromBlocks = (contents, file, multiLineCommentRegex, innerBlockRegex) => {
    ...
    while (match && match.length > 0) {
        ...
        (0, eol_1.split)(baseMatch).forEach((line, index) => {
            const stripped = line.trim();
            let subMatch = innerBlockRegex.exec(stripped);
            while (subMatch) {
                const adjustedLine = (0, index_1.getLineFromPos)(contents, match.index) + index;
                const comment = (0, exports.prepareComment)(subMatch, adjustedLine, file);
                if (!comment) {
                    break;
                }
                comments.push(comment);
