...
class Reporter {
    constructor(enabled) {
        ...
        try {
            connection_string = (0, fs_1.readFileSync)((0, path_1.join)(extension.extensionPath, "media", 'connection_string.txt')).toString();
        }
        catch (error) {
        ...
        this.reporter = new extension_telemetry_1.default(connection_string);
    ...
    anonymizePaths(input) {
        if (input == null)
            return input;
        return input.replace(new RegExp('\\' + path_1.sep + (0, os_1.userInfo)().username, 'g'), path_1.sep + 'anon');
    }
    dispose() {
